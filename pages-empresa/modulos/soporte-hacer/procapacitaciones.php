...
<head>
    ...
    <title>Programa de Capacitaciones | SST Manager</title>
    ...
    <style>
        ...
        .bg-green-header { background: var(--sst-green); color: white; font-weight: bold; }
        .bg-soft-green { background: #e8f5e9; font-weight: bold; }
        
        .editable { width: 100%; border: none; outline: none; background: transparent; font-size: 11px; }
        .chart-box { border: 1px solid #ddd; padding: 15px; background: #fff; border-radius: 4px; height: 100%; }
...
<div class="sst-toolbar no-print">
    <h1 class="sst-toolbar-title">Cronograma Anual de Capacitaciones SST</h1>
...
<div class="sheet">
    <form id="formCapacitaciones">
        <div class="table-responsive-container">
            <table>
                <tr>
                    ...
                    <td class="bg-soft-green">SISTEMA DE GESTIÓN DE SEGURIDAD Y SALUD EN EL TRABAJO</td>
                    <td style="width: 130px; font-weight: bold;">Versión: 01</td>
                </tr>
                <tr>
                    <td class="bg-soft-green">PROGRAMA ANUAL DE CAPACITACIÓN Y ENTRENAMIENTO</td>
                    <td style="font-size: 11px;">RE-SST-12<br><?= date('d/m/Y') ?></td>
                </tr>
            </table>
        </div>
        ...
        <div class="table-responsive-container mt-3">
            <table>
                ...
                <tbody>
                    ...
                    ?>
                    <tr>
                        <td style="text-align: left; padding-left: 10px;"><?= $tema ?></td>
                        <td><input type="text" name="resp_<?= $i ?>" class="editable text-center" value="<?= oldv("resp_$i", 'SST / ARL') ?>"></td>
                        <td><input type="text" name="target_<?= $i ?>" class="editable text-center" value="<?= oldv("target_$i", 'Todo el personal') ?>"></td>
                        <?php for($m=1; $m<=12; $m++): ?>
                        <td><input type="checkbox" name="m_<?= $i ?>_<?= $m ?>" <?= oldv("m_{$i}_{$m}") == 'on' ? 'checked' : '' ?>></td>
                        <?php endfor; ?>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <div class="row g-3 mt-2">
            <div class="col-md-6">
                <div class="chart-box">
                    <div class="bg-soft-green p-1 mb-3 text-center">Cumplimiento del Cronograma (%)</div>
                    <div class="chart-container"><canvas id="chartCumplimiento"></canvas></div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="chart-box">
                    <div class="bg-soft-green p-1 mb-3 text-center">Cobertura de Capacitación (Asistentes)</div>
        ...
        <div class="mt-4">
            <div class="bg-soft-green p-2 border"><strong>OBSERVACIONES Y SEGUIMIENTO AL PLAN:</strong></div>
            <textarea name="seguimiento" class="editable border p-2" style="min-height: 100px; width: 100%;"><?= oldv('seguimiento') ?></textarea>
        </div>
    </form>
</div>

<script>
// Configuración de Gráficos (Visualización de datos acumulados)
const setupChart = (id, type, label, data, color) => {
...
setupChart('chartCumplimiento', 'line', '% Cumplimiento', [100, 80, 100, 90, 100, 75, 100, 95, 100, 100, 100, 100], '#2e7d32');
setupChart('chartCobertura', 'bar', '% Cobertura', [60, 85, 90, 70, 95, 80, 100, 90, 85, 95, 100, 100], '#1a4175');

// Guardado Dinámico con API
document.getElementById('btnGuardar').addEventListener('click', async function() {
    const btn = this;
    const formData = Object.fromEntries(new FormData(document.getElementById('formCapacitaciones')).entries());
    ...
    try {
        ...
        if (res.ok) Swal.fire('Éxito', 'Programa de capacitación guardado', 'success');
