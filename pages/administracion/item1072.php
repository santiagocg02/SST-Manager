...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $idCategoriasTipo = (int)($_POST["id_categorias"] ?? 0);
...
<div class="container-fluid">
    ...
    <form method="POST" id="formItem1072" class="bg-white p-4 rounded card-shadow mb-4 border">
        ...
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-3">
                <label class="fw-bold small text-muted">VALOR</label>
                <input type="number" step="0.01" id="evaluacion" name="evaluacion" class="form-control" placeholder="Ej: 5.0">
            </div>
        </div>
    ...
    <div class="card-shadow border mb-4 rounded overflow-hidden">
        <div class="table-responsive w-100">
            <table class="table table-hover align-middle mb-0" style="min-width: 1200px;">
                ...
                <?php if(empty($lista)): ?>
                    <tr>
                        <td colspan="10" class="text-center text-muted py-4">No hay registros.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach($lista as $row): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-center fw-bold"><?= htmlspecialchars($row["evaluacion"] ?? "") ?></td>
                            <td class="text-center"><span class="<?= $estadoCls ?>"><?= $estadoTxt ?></span></td>
                            <td class="text-center">
                                <button type="button" class="btn btn-sm btn-light border shadow-sm" onclick="cargar('<?= base64_encode(json_encode($row)) ?>')">
...
const CICLOS_OK = CICLOS.filter(x => true);
